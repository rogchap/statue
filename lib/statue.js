  ...
  http.createServer(function(req, res){
    ...
    fs.stat(filePath, function(err, stats){
      ...
      if(stats.isDirectory()) {
        ...
        var options = {
            version:pkg.version,
            path:path.normalize(filePath.replace(dir,'/')),
            parentPath:path.normalize(path.dirname(filePath.replace(dir,'/'))),
            size:stats.size,
            mtime:stats.mtime,
            children:[]
          };
        ...
        fs.readdir(filePath, function(err, files){
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            options.children.push({
              name:child,
              path:path.normalize(childPath.replace(dir,'/')),
              size:stats.size,
              mtime:stats.mtime,
          ...
          util.debug(path.resolve(__dirname, 'statue-dir','directory.html'));
